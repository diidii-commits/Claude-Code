# -*- coding: utf-8 -*-
"""
무이성형외과 블로그 초안 리라이트 — 나이대별 성형 (성형학개론 영상 기반)
원본: 업로드된 CLI 초안 / 리라이트 기준: 224333520810 문체 + 세션에서 검증한 상위노출 규칙
...
GREEN = RGBColor(0x24, 0x5B, 0x12)   # 무이 키그린(소제목용)
...
def para(doc, text="", size=11, bold=False, color=BODY, align=None,
         before=3, after=7, italic=False, line=1.5):
    p = doc.add_paragraph()
...
# ── 상단 메타
para(doc, "무이성형외과 · 「성형학개론」 영상 기반 — 나이대별 성형 (리라이트 v2)", size=10.5, color=MUT, after=2, before=0)
...
para(doc, "검색결과에서 잘리지 않는 길이(30자 내). '나에게 맞는 ~ 찾는 법'은 이 블로그 1,138회 글의 검증된 제목 패턴. "
          "예비 제목: 「20대 30대 40대 성형, 나이대별로 순서 정하는 법」", size=9.5, color=MUT, after=10)
divider(doc)
...
para(doc, "상담실에서 정말 자주 듣는 고민이에요. 가만히 들어보면 부위가 고민이 아니라, 순서가 고민이신 거거든요.", before=9)
para(doc, "결론부터 말씀드리면, 나이대별 성형에 정해진 정답표는 없습니다. "
          "다만 상담실에서 보면 나이대마다 고민의 중심이 옮겨가는 흐름은 분명히 있어요. "
          "10대와 20대는 눈, 20대 후반부터는 윤곽과 재수술, 40대 이후로는 눈꺼풀과 처짐. "
          "이 흐름을 지도처럼 펼쳐놓고 보면, 지금 내 나이에서 뭘 먼저 봐야 할지 감이 잡히실 거예요.", bold=True)
para(doc, "오늘은 원장님이 상담실에서 직접 봐온 나이대별 흐름을 영상 내용 그대로 풀어드릴게요. "
          "개인의 경험을 바탕으로 정리한 이야기라, 사람마다 순서와 필요는 얼마든지 다를 수 있다는 점은 먼저 말씀드립니다.")

capture(doc, "0:15", "원장이 화이트보드에 남녀 나이대별 표를 그려놓고 설명을 시작하는 장면",
        "나이대별 성형 상담 관심사를 성별과 연령대로 정리한 표를 설명하는 장면")

heading(doc, "한눈에 보는 나이대별 성형 지도")
para(doc, "먼저 전체 그림부터 보여드릴게요. 아래 표는 상담실에서 관찰되는 '관심의 중심'이지, 권장 순서표가 아니에요.", after=6)
table(doc, [
    ["나이대", "여성 — 관심의 중심", "남성 — 관심의 중심"],
    ["10대", "눈 (쌍꺼풀), 일부는 코와 얼굴형까지", "관심 자체가 아직 적은 시기"],
    ["20대", "눈 구체화(재교정 요구), 사각턱 보톡스", "연애를 계기로 시작 — 무쌍, 눈매교정, 코"],
    ["20대 후반~30대", "윤곽 첫 수술 + 쌍꺼풀 재수술, 시술 병행 시작", "상대적으로 조용한 시기"],
    ["40~50대", "상안검, 하안검, 거상 — 시술 한계를 느끼는 시점", "50대에 상안검, 하안검, 거상 (가족과 함께)"],
], widths=[3.2, 6.6, 6.2])
...
para(doc, "여성분들은 청소년기부터 이미 고민이 시작돼요. 친구들끼리 거울을 보면서 \"너는 이 쌍꺼풀이 어울린다\", "
          "\"너는 여기를 해야 하는 거 아니냐\" 같은 이야기, 다들 한 번쯤 해보셨을 거예요. 그래서 10대는 눈에 대한 관심이 가장 큽니다.")
para(doc, "한 발 더 나간 친구들은 코까지 봐요. 면봉으로 콧대를 눌러보면서 서로 봐주기도 하고요. "
          "일부는 이때부터 사각턱이나 광대 같은 얼굴형 이야기를 시작합니다.")
...
para(doc, "재밌는 건, 이 시기에 이미 본인이 답을 알고 있다는 거예요. 내가 눈이 문제인지, 코인지, 광대나 사각턱인지. "
          "그래서 상담을 와서 새로 발견하기보다, 확인받으러 오시는 경우가 훨씬 많습니다. 혹시 지금 마음속에 짚이는 부위가 있으시다면, 아마 그게 맞을 거예요.")
para(doc, "20대가 되면 큰 틀은 그대로인데 결이 달라져요. 화장이라는 무기가 생기거든요. 셰딩으로 얼굴형을 보완하고, 아이라인으로 눈매를 잡고요. "
          "그래서 요구가 훨씬 구체적으로 바뀝니다. \"아이라인이 지워지니까 쌍꺼풀을 조금 키우고 싶어요\", \"눈밑을 정돈하고 싶어요\" 같은 식으로요.")

heading(doc, "20대 후반 30대 성형, 조합이 달라지는 시기예요")
para(doc, "얼굴형 쪽은 사각턱 보톡스를 이미 맞아보신 분들이 많아요. 보톡스로 한계가 느껴지면 그때부터 윤곽 성형을 고민하기 시작하고, "
          "사각턱과 함께 광대도 같이 보게 됩니다. 실제로 윤곽 상담을 가장 많이 오시는 시기가 20대 후반에서 30대예요.")
para(doc, "그리고 이때 새로 등장하는 게 쌍꺼풀 재수술입니다. 비절개로 잡았던 라인이 낮아지기도 하고, 절개로 했는데 모양이 아쉬운 경우도 있거든요.")
...
para(doc, "그래서 이 시기 상담은 조합이 독특해요. 윤곽은 처음인데, 눈은 재수술. 이런 분들이 정말 많습니다. "
          "경제적으로 여유가 생기면서 울쎄라나 써마지 같은 시술 정보도 이미 다 알고 오시고요. 수술과 시술을 함께 저울질하기 시작하는 시기라고 보시면 돼요.")
...
para(doc, "계기는 대체로 비슷해요. 울쎄라나 써마지를 300샷 정도 받을 때는 변화가 느껴졌는데, "
          "몇 년 지나 600샷, 800샷을 해도 예전 같지 않다고 느껴지는 순간이 오거든요. "
          "그때 흉터 걱정과 함께 수술적인 방법을 처음 고민하게 됩니다.")
para(doc, "혹시 지금 '시술을 해도 예전만큼 달라지지 않는다'고 느끼고 계시다면, 그게 하나의 신호일 수 있어요. "
          "여기서부터는 관리라는 관점에서 길게 보셔야 하는 시기입니다.")
...
para(doc, "이때 나오는 이야기가 무쌍 성형, 눈매교정, 코, 그리고 얼굴 크기 정리예요. "
          ...
          "남성분들은 \"그렇게 하면 별로다\" 한마디에 방향을 바꾸시는 경우가 많아요.")
para(doc, "30대에는 오히려 조용해집니다. 결혼을 바라보는 시기가 되면서 외모 고민의 비중이 줄어들거든요. "
          "이 시기 남성 상담은 배우자나 여자친구를 따라왔다가 \"제 눈밑도 좀 어두운가요?\" 하고 물어보시는 형태가 대부분이에요.")
...
para(doc, "그리고 50대에 다시 등장합니다. 상안검, 하안검, 거상이 함께 언급되는데, 배우자분과 함께 오셔서 상담받는 형태가 흔해요. "
          "결정도 빠른 편입니다. 방향이 잡히면 오래 고민하지 않고 진행하시는 분들이 많고, 회복 과정을 감수하면서 관리하시는 남성분들도 요즘은 꽤 늘었습니다.")
...
para(doc, "나이대별 흐름은 어디까지나 경향이에요. 개인의 뼈 구조와 피부 상태, 이전 수술 이력에 따라 필요한 방법과 시기는 완전히 달라집니다. "
          "특히 재수술이 포함되거나 눈꺼풀, 처짐이 얽힌 경우에는 진찰과 검사 없이 순서를 정하기 어려워요. "
          "그래서 순서를 정하는 일도 결국, 정확한 진단에서 시작됩니다.")

heading(doc, "자주 묻는 질문 (FAQ)")
faq(doc, "나이대별 성형에 정해진 순서가 있나요?",
    "정해진 표는 없어요. 다만 상담 현장에서는 눈, 윤곽과 재수술, 눈꺼풀과 처짐 순으로 관심이 옮겨가는 경향이 관찰됩니다. "
    "순서는 지금 본인이 가장 불편하게 느끼는 부위를 기준으로 잡으시는 게 현실적입니다.")
...
faq(doc, "남자도 상안검이나 하안검 상담을 많이 받나요?",
    "50대 전후로 문의가 늘어나는 편이에요. 다만 남성은 눈꺼풀 두께와 피부 특성이 달라 접근이 같지 않으니, 개별 진찰로 방향을 잡으셔야 합니다.")

heading(doc, "정리하면,")
para(doc, "나이대별 성형은 '몇 살에 뭘 해야 한다'의 문제가 아니라, '지금 내가 어디에서 불편함을 느끼고 있는가'의 문제예요.")
para(doc, "10대와 20대는 눈, 20대 후반부터는 윤곽과 재수술, 40대 이후로는 눈꺼풀과 처짐. "
          "이 흐름은 참고용 지도일 뿐이고, 실제 순서는 내 얼굴을 정확히 본 뒤에 정해집니다.")
para(doc, "무이성형외과에서는 특정 부위만 따로 보지 않고, 얼굴 전체 비율과 이전 수술 이력, 지금의 피부 상태까지 함께 확인한 뒤 우선순위를 정리해 드려요. "
          "혼자 순서를 고민하고 계셨다면, 진찰과 상담부터 시작해 보시길 권해드립니다.")
...
para(doc, "", after=2)
para(doc, "리라이트 메모", size=12, bold=True, color=ACCENT, after=4, before=10)
bullet(doc, "'비용은 누가 내는지' 섹션 삭제 — 검색 가치 없음 + 10대 아르바이트 언급은 의료광고 정서상 위험")
bullet(doc, "관찰 보고 톤('~가 관찰됩니다', '인상적인 건')을 독자 직접 호명 톤('혹시 지금 ~하고 계시다면')으로 전환")
bullet(doc, "나이대별 지도 표 신설(체류시간·가독성), 영상 임베드 위치 명시, 기존 글 3건 내부 링크(허브 글 전략)")
bullet(doc, "제목 33자 → 30자 내로 압축, 1,138회 글의 '나에게 맞는 ~ 찾는 법' 패턴 유지")
